...
class TestE2EPermutationTest:
    ...
    @pytest.mark.slow
    def test_permutation_test_hpc_mode(
        self, significance_proteomics_data, importance_enabled_config, tmp_path
    ):
        """
        Test: HPC single-permutation mode (--perm-index).

        Validates:
        - Single permutation runs with --perm-index
        - Individual perm_N.joblib file is generated
        """
        splits_dir = tmp_path / "splits"
        results_dir = tmp_path / "results"
        splits_dir.mkdir()
        ...
        # Run HPC mode (single permutation)
        result_perm = runner.invoke(
            cli,
            [
                "permutation-test",
        ...
        if result_perm.exit_code != 0:
            pytest.skip(f"HPC permutation failed: {result_perm.output[:500]}")

        assert result_perm.exit_code == 0

        # Verify single permutation output
        significance_dir = run_dirs[0] / "LR_EN" / "significance"
        perm_file = significance_dir / "perm_0.joblib"

        assert perm_file.exists(), f"perm_0.joblib not found in {significance_dir}"

        # Validate joblib content
        import joblib

        perm_result = joblib.load(perm_file)
        assert "null_auroc" in perm_result
        assert "perm_index" in perm_result
        assert perm_result["perm_index"] == 0
