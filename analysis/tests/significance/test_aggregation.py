...
- load_hpc_permutation_results: loading joblib files
- detect_and_aggregate: integration test for auto-detection
"""
...
# =============================================================================
# Tests: load_hpc_permutation_results
# =============================================================================


class TestLoadHpcPermutationResults:
    """Tests for load_hpc_permutation_results function."""

    def test_loads_joblib_files(self, tmp_path):
        """Test loading permutation results from joblib files."""
        # Create mock perm_*.joblib files
        for i in range(5):
            result = {
                "model": "LR_EN",
                "split_seed": 0,
                ...
                "perm_index": i,
                "perm_seed": 1000 + i,
                "null_auroc": 0.5 + np.random.rand() * 0.1,
            }
            joblib.dump(result, tmp_path / f"perm_{i:04d}.joblib")

        df = load_hpc_permutation_results(tmp_path)

        assert len(df) == 5
        assert "model" in df.columns
        assert "null_auroc" in df.columns
        assert "perm_index" in df.columns

    def test_returns_empty_df_for_nonexistent_dir(self, tmp_path):
        """Test returns empty DataFrame for nonexistent directory."""
        nonexistent = tmp_path / "nonexistent"

        df = load_hpc_permutation_results(nonexistent)

        assert df.empty

    def test_returns_empty_df_when_no_perm_files(self, tmp_path):
        """Test returns empty DataFrame when no perm_*.joblib files exist."""
    ...
    def test_handles_corrupted_files(self, tmp_path):
        """Test handles corrupted joblib files gracefully."""
        ...
        joblib.dump(
            ...
            tmp_path / "perm_0000.joblib",
        )
        # Create one corrupted file
        (tmp_path / "perm_0001.joblib").write_text("not a valid joblib")

        df = load_hpc_permutation_results(tmp_path)

        assert len(df) == 1  # Only the valid file loaded

    def test_handles_unexpected_format(self, tmp_path):
        """Test handles unexpected file formats gracefully."""
        # Create file with unexpected format (list instead of dict)
        joblib.dump([1, 2, 3], tmp_path / "perm_0000.joblib")
        # Create valid file
        joblib.dump(
            {
                "model": "LR_EN",
            ...
            tmp_path / "perm_0001.joblib",
        )

        df = load_hpc_permutation_results(tmp_path)

        assert len(df) == 1  # Only the valid dict loaded

    def test_sorts_results(self, tmp_path):
        """Test that results are sorted by model, seed, fold, perm_index."""
        ...
        assert list(df["perm_index"]) == [0, 1, 2, 3, 4]


# =============================================================================
# Tests: detect_and_aggregate
# =============================================================================


class TestDetectAndAggregate:
    """Tests for detect_and_aggregate function."""

    def test_detects_and_aggregates_model(self, tmp_path):
        """Test auto-detection and aggregation of single model."""
        # Create directory structure: run_dir/LR_EN/significance/perm_*.joblib
        model_dir = tmp_path / "LR_EN"
        ...
        for i in range(10):
            ...
            joblib.dump(result, sig_dir / f"perm_{i:04d}.joblib")

        results = detect_and_aggregate(tmp_path, alpha=0.05)

        assert "LR_EN" in results
        assert isinstance(results["LR_EN"], PooledNullResult)
        assert results["LR_EN"].n_perms_total == 10

    def test_aggregates_multiple_models(self, tmp_path):
        """Test aggregation of multiple models."""
        for model in ["LR_EN", "RF"]:
            sig_dir = tmp_path / model / "significance"
            sig_dir.mkdir(parents=True)
    ...
    def test_filters_to_specific_model(self, tmp_path):
        ...
        results = detect_and_aggregate(tmp_path, model="RF", alpha=0.05)
    ...
    def test_returns_empty_for_nonexistent_dir(self, tmp_path):
        ...
        results = detect_and_aggregate(nonexistent)

        assert results == {}

    def test_returns_empty_when_no_model_dirs(self, tmp_path):
        """Test returns empty dict when no model directories exist."""
        # Create empty run directory
        results = detect_and_aggregate(tmp_path)

        assert results == {}

    def test_skips_models_without_significance_dir(self, tmp_path):
        """Test skips model directories without significance/ subdirectory."""
        # Create model dir without significance/
        model_dir = tmp_path / "LR_EN"
        model_dir.mkdir()
    ...
    def test_skips_hidden_directories(self, tmp_path):
        ...
        hidden_dir = tmp_path / ".hidden" / "significance"
    ...
    def test_custom_alpha(self, tmp_path):
        """Test custom alpha is passed through to results."""
        sig_dir = tmp_path / "LR_EN" / "significance"
        sig_dir.mkdir(parents=True)
        ...
        results = detect_and_aggregate(tmp_path, alpha=0.01)

        assert results["LR_EN"].alpha == 0.01
