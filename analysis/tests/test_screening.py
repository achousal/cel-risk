...
def test_variance_missingness_prefilter_basic():
    ...
    kept, report = variance_missingness_prefilter(
        X, protein_cols, min_nonmissing=0.95, min_var=1e-6, strict=False
    )

    # Should keep only proteins with >=95% non-missing and variance > 1e-6
    assert len(kept) == 2
    assert "good_protein_resid" in kept
    assert "another_good_resid" in kept
    assert "low_var_protein_resid" not in kept
    assert "high_missing_protein_resid" not in kept

    # Check report structure
    assert len(report) == len(protein_cols)
...
def test_variance_missingness_prefilter_strict_mode():
    ...
    with pytest.raises(ValueError, match="failed variance filter"):
        variance_missingness_prefilter(
            X, protein_cols, min_nonmissing=0.5, min_var=1.0, strict=True
        )
...
def test_variance_missingness_prefilter_nonstrict_fallback():
    ...
    kept, report = variance_missingness_prefilter(
        X, protein_cols, min_nonmissing=0.5, min_var=1.0, strict=False
    )

    # Should return proteins that passed missingness even if variance fails
    assert len(kept) == 2
    # In non-strict mode, when all fail variance, they're kept anyway
    # so passed_variance shows True (they passed the fallback logic)
    assert all(passed for passed in report["passed_variance"])


def test_variance_missingness_prefilter_empty_input():
    """Test prefilter with empty protein list."""
    X = pd.DataFrame()
    protein_cols = []

    kept, report = variance_missingness_prefilter(X, protein_cols)
