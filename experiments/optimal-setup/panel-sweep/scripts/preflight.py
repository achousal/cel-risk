...
def generate_configs(orders: list[str] | None = None) -> None:
    ...
    for order_name, order in target_orders.items():
        for size in range(PANEL_MIN, PANEL_MAX + 1):
            ...
            # fixed_panel_csv resolves relative to data/ directory (see feature_stage.py)
            # Use ../ to escape data/ into the project root, then into experiments/
            panel_rel = f"../experiments/optimal-setup/panel-sweep/panels/{order_name}_{size}p.csv"
            config_content = (
