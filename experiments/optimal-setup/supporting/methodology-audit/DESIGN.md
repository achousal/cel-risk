...
This directory contains scripts for empirical validation of methodological choices flagged in the mathematical validity audit. That audit note is not currently centralized under `docs/dev/notes/`, so this directory carries the execution-side context.
...
After running these studies, update the mathematical validity audit note under `docs/dev/notes/`:
