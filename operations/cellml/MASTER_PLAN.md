...
analysis/
├── configs/
│   ├── manifest.yaml                       # declarative recipe source of truth
│   ├── training_config.yaml                # base training params (inherited, not copied)
│   ├── pipeline_hpc.yaml                   # base pipeline params (inherited, not copied)
│   ├── splits_config.yaml                  # base splits (V0 will overlay)
...
└── src/ced_ml/recipes/                     # recipe derivation code
...
2. **No standalone experiment scripts.** All training/evaluation runs through `ced run-pipeline`. Factorial-specific assets (compile, validate, submit) live in `factorial/`.
...
As specified in `factorial/DESIGN.md`:
...
|---|---|---|
| Cell manifest | `analysis/configs/recipes/cell_manifest.csv` | `ced derive-recipes` |
| Derived panels | `analysis/configs/recipes/{recipe_id}/panel.csv` | `ced derive-recipes` |
| Size audit logs | `analysis/configs/recipes/{recipe_id}/size_derivation.json` | `ced derive-recipes` |
...
| Component | File | Purpose |
|---|---|---|
| Program | `factorial/analysis/factorial_analysis_program.md` | V1-V5 instructions, dataset-agnostic |
| Theme | `factorial/analysis/_theme_factorial.R` | Dynamic RECIPE_COLORS, `load_factorial()` helper |
| Runner | `factorial/analysis/runner.py` | R execution harness + artifact logging |
| V1 heatmap | `factorial/analysis/scripts/v1_01_recipe_heatmap.R` | Recipe × model AUROC heatmap |
| V2 Pareto | `factorial/analysis/scripts/v2_01_pareto_front.R` | AUROC-Brier Pareto front |
