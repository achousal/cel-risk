...
Pareto minimum = smallest panel_size where p_adj < alpha for that model.
...
def compute_significance(
    ...
    # Pareto minimum per model: smallest panel_size that is significant
    pareto = (
        out[out["significant"]]
    ...
    out = out.merge(pareto, on="model", how="left")
...
def plot(results: pd.DataFrame, prevalence: float, alpha: float, out_dir: Path) -> None:
    ...
    for ax, model in zip(axes, models):
        ...
        ax.plot(xs[~sig], ys[~sig], "o", color="gray", ms=6, zorder=3, label="p_adj ≥ {:.2f}".format(alpha))
        ax.plot(xs[sig], ys[sig], "o", color="steelblue", ms=7, zorder=4, label="p_adj < {:.2f}".format(alpha))
        ax.plot(xs, ys, "-", color="steelblue", lw=1.2, zorder=2)

        # Null baseline
        ax.axhline(prevalence, color="tomato", lw=1.2, ls="--", label=f"null (prevalence={prevalence:.4f})")

        # Pareto minimum
        if not np.isnan(pareto_min):
            ax.axvline(pareto_min, color="darkgreen", lw=1.5, ls=":", label=f"pareto min N={int(pareto_min)}")
    ...
    fig.suptitle("Purity panel: AUPRC significance vs chance (BH-FDR)", fontsize=12, y=1.01)
    fig.tight_layout()
...
def main() -> None:
    ...
    results = compute_significance(df, prevalence, args.alpha)

    out_path = args.out / "purity_significance.csv"
    results.to_csv(out_path, index=False)
    log.info("Saved %s", out_path)

    plot(results, prevalence, args.alpha, args.out)

    print(f"\n=== Purity panel significance (alpha={args.alpha}, prevalence={prevalence:.5f}) ===")
    print(results[[
        "model", "panel_size", "test_auprc", "p_value", "p_adj", "significant", "pareto_min"
    ]].to_string(index=False, float_format=lambda x: f"{x:.4f}"))

    print("\n=== Pareto minima ===")
    pareto = results[results["significant"]].groupby("model")[["panel_size", "test_auprc", "p_adj"]].first()
    print(pareto.to_string(float_format=lambda x: f"{x:.4f}"))
