#!/usr/bin/env python3
"""
Unified incident validation pipeline: 3 models x 3 strategies x 4 weights.

Models:
  LR_EN  -- ElasticNet logistic regression (tune C + l1_ratio)
  SVM_L1 -- LinearSVC L1 + CalibratedClassifierCV (tune C, sparse)
  SVM_L2 -- LinearSVC L2 + CalibratedClassifierCV (tune C, dense)

Workflow:
  1. Load data -> define groups (incident / prevalent / control)
  2. Create locked 20% test set (incident + controls, stratified by sex)
  3. Bootstrap stability feature selection on dev incident + controls
     (Wald statistic, 100 resamples, top 200/resample, >=70% threshold)
  4. Correlation pruning (|r| > 0.85, keep more stable feature)
  5. 5-fold CV on dev set (folds defined on incident + controls)
     - For each (strategy x weight_scheme):
       inner 3-fold Optuna tunes hyperparams -> AUPRC
...
Usage:
  cd cel-risk
  python operations/incident-validation/scripts/run_lr.py --model LR_EN
  python operations/incident-validation/scripts/run_lr.py --model SVM_L1
  python operations/incident-validation/scripts/run_lr.py --model SVM_L2
  python operations/incident-validation/scripts/run_lr.py --model LR_EN --smoke

Parallel mode (HPC):
  # Phase 1: feature selection (shared across combos)
  python run_incident_validation.py --model SVM_L1 --phase features

  # Phase 2: one job per (strategy, weight) combo
  python run_incident_validation.py --model SVM_L1 --phase cv --strategy incident_only --weight-scheme balanced

  # Phase 3: aggregate combo results, refit, test
  python run_incident_validation.py --model SVM_L1 --phase aggregate
"""
...
# Unbuffered stderr for LSF/SLURM live progress
...
class Config:
    ...
    # Bootstrap stability (LOCKED -- same for all models)
    n_bootstrap: int = 100
    ...
    # Model-specific defaults (overridden per model)
    max_iter: int = 2_000
    solver: str = "saga"  # LR only
    calibration_cv: int = 5  # SVM only
...
# ============================================================================
# Optuna tuning + outer fold evaluation
# ============================================================================
...
def run_cv(
    cfg: Config, data: dict, features: dict, model_spec: ModelSpec
) -> tuple[pd.DataFrame, list]:
    """Run 5-fold CV for all (strategy x weight) combinations."""
    df = data["df"]
...
def save_results(
    ...
    # 8. Summary report
    report = _build_report(cfg, data, features, cv_results, final, model_spec)
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified incident validation pipeline (LR_EN / SVM_L1 / SVM_L2)"
    )
    parser.add_argument(
        "--model",
        type=str,
        ...
        help="Model to run: LR_EN, SVM_L1, or SVM_L2",
    )
    parser.add_argument(
        "--smoke", action="store_true", help="Reduced params for quick test"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Pipeline phase: all (sequential), features, cv, aggregate",
    )
    parser.add_argument(
        "--strategy", type=str, default=None, help="Single strategy for --phase=cv"
    )
    parser.add_argument(
        "--weight-scheme",
        type=str,
        default=None,
        help="Single weight scheme for --phase=cv",
    )
    ...
    # Set default output dir per model
    if args.output_dir:
        cfg.output_dir = Path(args.output_dir)
    else:
        cfg.output_dir = Path(MODEL_OUTPUT_DIRS[args.model])
    ...
    logger.info("=" * 60)
    logger.info(
        "INCIDENT VALIDATION PIPELINE -- %s (phase=%s)", args.model, args.phase
    )
    logger.info("=" * 60)
    if cfg.smoke:
    ...
    if args.phase == "all":
        ...
        cv_results, fold_coefs = run_cv(cfg, data, features, model_spec)
        final = final_refit_and_test(cfg, data, features, cv_results, model_spec)
        save_results(cfg, data, features, cv_results, final, fold_coefs, model_spec)
        ...
        pd.DataFrame(coef_rows).to_csv(
            combo_dir / f"coefs_{combo_tag}.csv", index=False
        )
        ...
        cv_results = pd.concat(
            [pd.read_csv(f) for f in cv_parts], ignore_index=True
        )
        logger.info(
            "Merged %d combo files -> %d CV rows", len(cv_parts), len(cv_results)
        )

        fold_coefs_df = pd.concat(
            [pd.read_csv(f) for f in coef_parts], ignore_index=True
        )
        ...
        save_results(
            cfg, data, features, cv_results, final, fold_coefs_list, model_spec
        )
